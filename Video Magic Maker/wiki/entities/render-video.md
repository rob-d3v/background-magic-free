---
title: render-video — Render offline (RVM/MediaPipe), troca de fundo sem GPU
type: entity
created: 2026-06-14
updated: 2026-06-22
...
status: stable
migrated-from: wiki/components/render-video.md
...
([[entities/remocao]]) e **não reilumina** (sem IC-Light) — troca o fundo com
...
| `render_arquivo(...)` | **arquivo de vídeo** (lê direto, sem extrair frames) | um **mp4** | **remuxa o áudio original** | botão 🎬 do [[entities/camera-app]] |
...
melhor que o [[entities/composicao|compor]] (rembg) e dispensa a GPU/Colab do
relight IC-Light ([[entities/relighting]]).
...
2. **Fundo de imagem ou de vídeo.** Se `background_path` termina em extensão de
   ...
   ([[entities/live-mode]]) e **avança 1 frame de fundo por frame de saída**
...
> frames **em ordem** dá menos tremor de borda — vantagem que o [[entities/live-mode|live]]
...
tudo)** do **modo vídeo** do [[entities/camera-app]].
...
- **Não passa por rembg.** O modo HD recorta direto do **frame cru** e **pula** o
  passo `remover_fundo` ([[entities/remocao]]).
- **Não reilumina.** A luz da pessoa continua a do vídeo original (como o
  [[entities/composicao|compor]] e o [[entities/live-mode|live]]); relight real
  só no modo IC-Light ([[entities/relighting]]).
...
- `MODO_COMPOR = "Compor (rapido, CPU)"` — rembg + [[entities/composicao]].
- `MODO_RELIGHT = "Reiluminar (IC-Light, GPU)"` — [[entities/relighting]].
...
- **Aplicar** (`cb_aplicar`): `shutil.rmtree(frames_relit)` → `render_matting(
  ...
  ([[entities/exportacao]], com o áudio original). **Pula** o `remover_fundo`.

## Integração no app de câmera (`camera_app.py`)
No **modo vídeo** do [[entities/camera-app]] (estado `source=="video"`), o vídeo
...
## Relacionados
[[concepts/rvm-matting]] · [[entities/camera-app]] · [[entities/live-mode]] ·
[[entities/composicao]] · [[entities/exportacao]] · [[entities/remocao]] ·
[[entities/relighting]] · [[concepts/gpu-vram-local-vs-colab]] ·
[[concepts/agent-render-video-offline-pipeline]] ·
[[decisions/agent-render-video-engine-forced-rvm]] ·
[[sources/agent-render-video-source]] · [[index]]
