...
date: 2026-06-13
status: stable
source: agentes/matting_live.py
---

# Matting em tempo real (MediaPipe Tasks + câmera virtual)

Conceito por trás do [[components/live-mode]]: segmentar a pessoa de cada frame da
...
- **rembg (`u2net_human_seg`)** é preciso porém pesado — bom para o lote offline
  ([[components/remocao]]), lento demais por frame ao vivo.
- **IC-Light** reilumina, mas relight real por frame é **inviável a 30fps em 4GB
  ...
  — só matting + composição. Relight fica no modo offline ([[components/relighting]]).
...
cover-crop da imagem de fundo (mesma lógica de [[components/composicao]]) **ou** a
...
**Feito nesta sessão:** o [[components/live-mode|live]] ganhou um segundo motor,
...
[[components/camera-app]]; flag `--engine {mediapipe,rvm}` no `live.py`), default
...
## Relacionados
[[concepts/rvm-matting]] · [[components/live-mode]] · [[components/composicao]] ·
[[components/relighting]] · [[concepts/rembg-background-removal]] ·
